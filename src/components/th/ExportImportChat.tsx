  ...
  const sendMessage = async () => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke('chat-export-import', {
        body: { message: userMessage.content, language: 'th' }
      });
      ...
      toast({
        title: "ข้อผิดพลาด",
        description: "ไม่สามารถส่งข้อความได้ กรุณาลองใหม่ภายหลัง",
        variant: "destructive",
      });
    } finally {
      setIsLoading(false);
      ...
      <div className="fixed bottom-6 right-6 z-50">
        ...
        >
          <div className="flex flex-col items-center">
            ...
            <span className="text-xs font-bold mt-1">แชท</span>
          </div>
        </Button>
      </div>
    );
      ...
      <Card className="w-96 h-[500px] shadow-xl border-0 bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/90">
        <CardHeader className="pb-3">
          ...
          <p className="text-sm text-muted-foreground">
            ถามอะไรก็ได้เกี่ยวกับการดำเนินงานนำเข้า/ส่งออกญี่ปุ่น
          </p>
        </CardHeader>
            ...
            <div className="space-y-4 pb-4">
              {messages.length === 0 && (
                <div className="text-center text-muted-foreground text-sm py-8">
                  ถามคำถามเกี่ยวกับนำเข้า/ส่งออกของคุณ!<br />
                  เช่น "ขั้นตอนการส่งออกอาหารไปสหรัฐฯ คืออะไร?"
                </div>
              )}
