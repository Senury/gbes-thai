...
- Partner Search deep-dive findings (pending action):
  ...
  - ✅`DataSourceSelector` test button never succeeds because client/server disagree on the test flag and response shape.
  ...
  - ✅ `CompanySearchService.scrapeCompanyWebsites` allows non-logged-in users to attempt scrapes; consider gating behind auth to prevent anonymous abuse.
